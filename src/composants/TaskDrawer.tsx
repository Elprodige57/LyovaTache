...
export function TaskDrawer({ task, columns, currentMember, allLabels = [], allDocuments = [], workspaceId }: TaskDrawerProps) {
  ...
  const handleDeleteTask = async () => {
    if (!(await confirmDialog('Supprimer cette tâche ?', { message: 'Cette action est irréversible.', danger: true }))) return;
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                      ...
                      <span style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
                        <button onClick={async () => { const t = await promptDialog('Modifier le commentaire', c.content); if (t !== null && t.trim()) app.updateComment(c.id, t.trim()); }} title="Modifier" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--sub2)', display: 'flex', padding: 2, borderRadius: 4 }} onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent-ink)')} onMouseLeave={e => (e.currentTarget.style.color = 'var(--sub2)')}>
                        ...
                        <button onClick={async () => { if (await confirmDialog('Supprimer ce commentaire ?', { danger: true })) app.removeComment(c.id, task.id); }} title="Supprimer" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--sub2)', display: 'flex', padding: 2, borderRadius: 4 }} onMouseEnter={e => (e.currentTarget.style.color = '#ef4444')} onMouseLeave={e => (e.currentTarget.style.color = 'var(--sub2)')}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6M10 11v6M14 11v6M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /></svg>
                        </button>
                      </span>
                    </div>
